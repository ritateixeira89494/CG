...
Translate::Translate(float time, bool alignment, vector<float *> points) : Transform(0,0,0) {
    full_time   = time;
...
Translate::Translate(float time, bool alignment) : Transform(0,0,0) {
    full_time   = time;
    curr_time   = milliseconds{0};
    //last_clock  = 0;
    align       = alignment;
...
void Translate::get_catmull_rom_point(float t, float *p0, float *p1, float *p2, float *p3, float *pos, float *deriv) {
    ...
    for(int i = 0; i < 3; i++) {
        ...
        pos[i] = T[0]*A[i][0] + T[1]*A[i][1] + T[3]*A[i][3];
...
void Translate::get_global_catmull_rom_point(float *pos, float *deriv) {
    ...
    float t = curr_time.count() * n_points;
    int index = floor(t);
    t = t - index;

    int indices[4];
    indices[0] = (index + n_points) % n_points;
